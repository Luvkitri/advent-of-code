...
impl CityMap {
    fn get_neighbors(&self, pointer: &CityPointer) -> Vec<CityPointer> {
        ...
        for direction in directions {
            ...
            if direction == pointer.direction && pointer.direction_count < 3 {
                neighbors.push(CityPointer {
                    position: (next_i, next_j),
                    direction,
                    direction_count: pointer.direction_count + 1,
                });
            } else if direction != pointer.direction || pointer.direction_count == 0 {
                neighbors.push(CityPointer {
                    position: (next_i, next_j),
                    direction,
                    direction_count: 1,
    ...
    fn is_end(&self, pointer: &CityPointer) -> bool {
        if pointer.position == (self.height - 1, self.width - 1) {
            return true;
        }
